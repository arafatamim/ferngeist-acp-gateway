...
**Contract versioning:** the gateway advertises its protocol version in
`GET /v1/status` (`protocolVersion`) and in the mDNS discovery TXT record
(`protocol_version`). The client's version gate checks it at connect. See
[`docs/changelog.md`](changelog.md) for the versioning policy and a record of
every contract-visible change; `CHANGELOG.md` (generated by git-cliff) has the
